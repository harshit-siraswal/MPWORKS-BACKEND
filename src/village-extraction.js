...
export function extractVillages(work) {
  ...
  const gramNamePattern = /\bGRAM\s+([A-Za-z\u0900-\u097F][A-Za-z0-9\u0900-\u097F.'’()\-/ ]{2,60}?)(?=\s+(?:ME|MAIN|MEIN|AT|IN|VIA)\b|\s*[,;.]|$)/gi;
  ...
  const locationPattern = /\b(?:at|near)\s+([A-Za-z\u0900-\u097F][A-Za-z0-9\u0900-\u097F.'’()\-/ ]{2,60}?)(?=\s*(?:,|;|\.|\(|\)|\b(?:block|district|tehsil|taluk|ward|under|road|via|opposite|opp\.?|temple|school|market)\b|$))/gi;
  for (const match of description.matchAll(gramNamePattern)) {
    const name = clean(match[1]);
    if (name.length >= 3) results.push({ name, normalizedName: normalizeVillageName(name), extractionMethod: 'description-gram-phrase', confidence: 0.86, rawContext: match[0] });
  }
  ...
  for (const match of description.matchAll(locationPattern)) {
    const name = clean(match[1]).replace(/\s+(?:and|or)$/i, '');
    if (name.length < 3) continue;
    results.push({ name, normalizedName: normalizeVillageName(name), extractionMethod: 'description-location-phrase', confidence: 0.5, rawContext: match[0] });
  }
