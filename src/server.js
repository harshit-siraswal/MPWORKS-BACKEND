...
const server = createServer(async (request, response) => {
  ...
  if (projectMatch && request.method === 'GET') {
    ...
    if (projectMatch[2] === 'evidence') {
      ...
      const comparisonComplete = cachedComparison?.status === 'completed';
      const storedFiles = publicFiles.filter((file) => file.r2Url);
      const storedComparison = cachedComparison || (publicFiles.length ? { status: 'queued', reason: 'Evidence is available; AI comparison has not been completed for this record.' } : { status: 'not-available', reason: 'No evidence comparison is available.' });
